...
class controller:
    ...
    def controller(self, data):
        ...
        if self.running < 10:
            ...
            self.mu_hat_log.append(pymorph.dec_hom(var.p, var.L, sk, [self.mu_hat])[0][0])
            import csv
            import os
            PATH = os.path.dirname(os.path.abspath(__file__))
            FILEPATH = os.path.join(PATH, 'mu_log'+self.name+'.csv')
            with open(FILEPATH, "w") as output:
                
                np.savetxt("foo"+self.name+".csv", (np.asarray(self.mu_hat_log)), delimiter=",")
            ...
            xy_by_mu = pymorph.hom_mul_mat(var.q, var.N, self.xy, [self.mu_hat, [0,0,0],[0,0,0]]) #zeros[0], zeros[0]])#self.mu2[0], self.mu2[0]])
	    ...
	    #print "Ed= ", Ed
            ...
            U.append(map(add, self.e2[0],self.e2[2]))
            Y.append(map(add, self.e2[1],self.e2[2]))
