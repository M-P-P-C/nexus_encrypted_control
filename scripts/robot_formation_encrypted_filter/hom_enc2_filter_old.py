...
class hom_encrypt:
    ...
    def __init__(self):    #The "self" structure replaces "env" from Kim's paper
        ...
        self.pub_noise = rospy.Publisher(self.name+'/noise', numpy_msg(Floats), queue_size=1)
    ...
    def enc(self, data):
        ...
        if self.running:
            ...
            print "actual u_x :", 0.5*(z_values[1]-0.8 )

            noise = np.zeros(2, dtype = np.float32)
            k=0
            for j in [1,2]:
                for i in [z_values[j]]:
                    noise[k] = round(i)-i+np.random.randint(low=0, high =2)
                    z_values[j] = i+noise[k]
                    k+=1

            self.pub_noise.publish(noise)
            ...
            while np.any(abs(z_values[:, 3])<=10):
            ...
            #Encryption of x and y distance
